...
app.get('/api/fatsecret/recipes', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (
      ...
      (resultsArray.length === 1 && !resultsArray[0])
    ) {
      ...
      resultsArray.map(async (r: any) => {
        ...
        try {
          ...
          return {
            ...
            title: r.recipe_name,
            image:
              r.recipe_image || 'https://placehold.co/312x231?text=No+Image',
...
app.get('/api/fatsecret/recipes/:id', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const ingredientsArray = Array.isArray(ingredientsRaw)
      ? ingredientsRaw
      : [ingredientsRaw];

    const directionsRaw = r.directions?.direction || [];
    const directionsArray = Array.isArray(directionsRaw)
      ? directionsRaw
      : [directionsRaw];
    ...
    const recipeDetail = {
      ...
      extendedIngredients: ingredientsArray.map((i: any) => ({
        original:
          i.recipe_ingredient_description ||
          `${i.number_of_units} ${i.measurement_description} ${i.food_name}`,
      })),
      instructions: directionsArray
        .map((d: any) => d.recipe_direction_description)
        .join('\n\n'),
